...
bl_info = {
    ...
    "version": (1, 1),
    "blender": (2, 92, 0),
    "location": "View3D > Object > Anti Mesh Tool",
...
class AddOn_AntiMesh_Property(bpy.types.PropertyGroup):
    ...
    mode_boolean: bpy.props.EnumProperty(name= "Effect ", items=type_boolean)
...
class VIEW3D_MT_mode_object_menu(bpy.types.Menu):
    bl_label = "Anti mesh"
    bl_icon = 'FUND'
    def draw(self, context):
        layout = self.layout
...
class VIEW3D_MT_MainMenu(bpy.types.Menu):
    bl_label = "Anti mesh"
    bl_icon = 'FUND'
    def draw(self, context):
        layout = self.layout
        layout.operator("object.apply_all_bool",icon='EXPERIMENTAL')
...
class VIEW3D_MT_booltoool_menu(bpy.types.Menu):
    bl_label = "Anti Mesh Tool"
    bl_idname = "VIEW3D_MT_booltoool_menu"

    def draw(self, context):
        layout = self.layout
        layout.label(text="Anti mesh")
        layout.operator("object.made_anti_cible",icon='SNAP_VERTEX')
        layout.operator("object.apply_all_bool",icon='EXPERIMENTAL')
        layout.separator()
        global bool_active
        
        layout.operator("object.active_bool_truc",text="OFF",icon='SHADING_RENDERED')
        layout.operator("object.active_bool_truc",text="ON",icon='SHADING_SOLID')


def VIEW3D_MT_addon_top_bar(self, context):
    self.layout.menu(VIEW3D_MT_booltoool_menu.bl_idname)
